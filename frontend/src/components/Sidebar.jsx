  ...
  return (
    <div style={{ width: '220px', background: t.sidebar, padding: '28px 16px', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 10, overflowY: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '40px', paddingLeft: '8px' }}>
        <div style={{ width: '38px', height: '38px', borderRadius: '12px', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '18px', boxShadow: '0 6px 20px rgba(59,130,246,0.35), inset 0 1px 1px rgba(255,255,255,0.2)' }}>S</div>
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px', flex: 1 }}>
        {NAV.map(n => (
          <button key={n.id} onClick={() => { setPage(n.id); if (n.id === 'insights') setInsightsSeen(true) }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '12px', border: 'none', cursor: 'pointer', width: '100%', textAlign: 'left', background: page === n.id ? `${t.tealDark}25` : 'transparent', color: page === n.id ? t.teal : '#8FA3B0', fontWeight: page === n.id ? 600 : 400, fontSize: '14px', transition: 'all 0.2s', borderLeft: page === n.id ? `3px solid ${t.teal}` : '3px solid transparent', boxShadow: page === n.id ? '0 0 20px rgba(59,130,246,0.15)' : 'none' }}
            onMouseEnter={e => { if (page !== n.id) e.currentTarget.style.background = 'rgba(255,255,255,0.05)' }} onMouseLeave={e => { if (page !== n.id) e.currentTarget.style.background = 'transparent' }}>
